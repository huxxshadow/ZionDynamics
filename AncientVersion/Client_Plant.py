...
def receiveLongMsg():  # receive the long message
...
def receiveShortMsg(len):  # receive the short message
    data = sock.recv(len)
    return data
...
def mp3_to_wav(mp3_path):  # convert the mp3 file to wav file
    sound = AudioSegment.from_mp3(mp3_path)
    print(sound.frame_rate)
    sound.export("received.wav", format="wav")


def play_wav(file_path):  # play the wav file
...
class keepMonitor(Thread):  # monitor the voice input
...
sock = socket.socket()
sock.connect(('172.28.169.86', 9009))  # connect to the server
